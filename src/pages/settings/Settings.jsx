...
import { Card, StatCard, Btn, Input, Badge, Empty, ErrorAlert } from '../../components/shared/UI';
          ...
          return (
            <div key={p.key} className="p-5 rounded-xl relative"
              style={{
                ...
                border: `1px solid ${isCurrent ? 'rgba(59,130,246,0.5)' : p.popular ? 'rgba(59,130,246,0.3)' : 'var(--border)'}`,
              }}>
              {p.popular && (
                <span className="absolute -top-2.5 left-4 text-[10px] font-bold px-2 py-0.5 rounded-full"
                  style={{background:'var(--blue)', color:'#fff'}}>Most popular</span>
              )}
              {isCurrent && (
                <span className="absolute -top-2.5 right-4 text-[10px] font-bold px-2 py-0.5 rounded-full"
                  style={{background:'var(--green)', color:'#fff'}}>Current</span>
              )}
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm font-bold" style={{color:'var(--text)'}}>{p.name}</span>
                <span className="text-xl font-black font-mono" style={{color:'var(--text)'}}>{p.price}<span className="text-xs font-normal text-gray-500">/mo</span></span>
              </div>
  ...
  return (
    <div className="p-4 md:p-6 max-w-2xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-6 p-4 rounded-xl" style={{background:'var(--surface)', border:'1px solid var(--border)'}}>
        ...
        <div>
          <div className="text-base font-bold" style={{color:'var(--text)'}}>{user?.first_name} {user?.last_name}</div>
          <div className="text-xs" style={{color:'var(--text2)'}}>{user?.email}</div>
          <div className="text-xs mt-0.5 capitalize" style={{color:'var(--text3)'}}>{user?.role} · {org?.name}</div>
        </div>
      </div>
...
export function ContractorsPage() {
  ...
  const [formData, setFormData] = React.useState({ firstName:'', lastName:'', email:'', visaType:'h1b', billRate:'', payRate:'' });
  ...
  return (
    <div className="p-4 md:p-6 max-w-4xl mx-auto">
      ...
      {showForm && (
        <Card className="mb-5">
          ...
          <div className="grid grid-cols-2 gap-3 mt-3">
            <Input label="First name" value={formData.firstName} onChange={e => setFormData(p=>({...p,firstName:e.target.value}))} placeholder="Alex" />
            <Input label="Last name"  value={formData.lastName}  onChange={e => setFormData(p=>({...p,lastName:e.target.value}))}  placeholder="Chen" />
            <Input label="Email"      value={formData.email}     onChange={e => setFormData(p=>({...p,email:e.target.value}))}     placeholder="alex@email.com" type="email" />
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] font-bold uppercase tracking-widest" style={{color:'var(--text3)'}}>Visa type</label>
              <select value={formData.visaType} onChange={e => setFormData(p=>({...p,visaType:e.target.value}))}
                className="rounded-lg px-3 py-2.5 text-sm outline-none"
                style={{background:'var(--elevated)', border:'1px solid var(--border)', color:'var(--text)'}}>
                {['h1b','opt','ead','gc','citizen','tn','other'].map(v => <option key={v} value={v}>{v.toUpperCase()}</option>)}
              </select>
            </div>
            <Input label="Bill rate ($/hr)" value={formData.billRate} onChange={e => setFormData(p=>({...p,billRate:e.target.value}))} placeholder="85" type="number" />
            <Input label="Pay rate ($/hr)"  value={formData.payRate}  onChange={e => setFormData(p=>({...p,payRate:e.target.value}))}  placeholder="65" type="number" />
          </div>
      ...
      {loading ? <div className="py-12 text-center text-sm" style={{color:'var(--text3)'}}>Loading…</div>
        : contractors.length === 0 ? <Empty icon="👤" title="No contractors yet" desc="Add your first contractor to start tracking placements." action={() => setShowForm(true)} actionLabel="Add contractor" />
        : (
            ...
            {contractors.map(c => (
              <div key={c.id} className="flex items-center gap-4 p-4 rounded-xl"
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-semibold" style={{color:'var(--text)'}}>{c.first_name} {c.last_name}</div>
                  <div className="text-xs" style={{color:'var(--text2)'}}>{c.email}</div>
                </div>
                <Badge label={c.visa_type?.toUpperCase()||'—'} type="blue" />
                <div className="text-xs font-mono text-right flex-shrink-0" style={{color:'var(--text3)'}}>
...
export function ClientsPage() {
  ...
  const [showForm, setShowForm] = React.useState(false);
  const [formData, setFormData] = React.useState({ name:'', contactName:'', contactEmail:'', industry:'technology' });
  const [saving, setSaving] = React.useState(false);
  const [err, setErr] = React.useState('');
    ...
    <div className="p-4 md:p-6 max-w-4xl mx-auto">
      ...
      {showForm && (
        <Card className="mb-5">
          <ErrorAlert message={err} />
          <div className="grid grid-cols-2 gap-3 mt-3">
            <Input label="Company name"    value={formData.name}         onChange={e => setFormData(p=>({...p,name:e.target.value}))}         placeholder="Acme Technologies" className="col-span-2" />
            <Input label="Contact name"    value={formData.contactName}  onChange={e => setFormData(p=>({...p,contactName:e.target.value}))}  placeholder="Jane Smith" />
            <Input label="Contact email"   value={formData.contactEmail} onChange={e => setFormData(p=>({...p,contactEmail:e.target.value}))} placeholder="jane@acme.com" type="email" />
          </div>
          <div className="flex gap-3 mt-4">
            <Btn onClick={() => setShowForm(false)} variant="ghost">Cancel</Btn>
      ...
      {loading ? <div className="py-12 text-center text-sm" style={{color:'var(--text3)'}}>Loading…</div>
        : clients.length === 0 ? <Empty icon="🏢" title="No clients yet" desc="Add your first client to start tracking health and revenue." action={() => setShowForm(true)} actionLabel="Add client" />
        : (
          <div className="space-y-2">
            {clients.map(c => (
              <div key={c.id} className="flex items-center gap-4 p-4 rounded-xl"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs" style={{color:'var(--text2)'}}>{c.primary_contact_name} · {c.primary_contact_email}</div>
                </div>
                <div className="text-xs font-mono text-right flex-shrink-0" style={{color:'var(--text3)'}}>
...
export function ReferralsPage() {
  ...
  return (
    <div className="p-4 md:p-6 max-w-2xl mx-auto">
      ...
      <p className="text-sm mb-6" style={{color:'var(--text2)'}}>Earn $20 credit for every IT staffing firm you refer to Plaxiq.</p>

      <Card className="mb-6">
        <div className="text-xs font-bold uppercase tracking-widest mb-2" style={{color:'var(--text3)'}}>Your referral link</div>
        <div className="flex items-center gap-3 p-3 rounded-lg" style={{background:'var(--elevated)', border:'1px solid var(--border)'}}>
          <span className="flex-1 text-sm font-mono truncate" style={{color:'var(--text)'}}>{referralLink}</span>
          <Btn size="sm" variant="ghost" onClick={() => navigator.clipboard.writeText(referralLink)}>Copy</Btn>
        </div>
      </Card>
