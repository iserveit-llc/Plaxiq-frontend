...
import { StatCard, SectionHead, Badge, agingBadgeType, ragType, Empty, PulseDot, TrialBanner } from '../../components/shared/UI';
  ...
  return (
    <button onClick={onClick}
      ...
      <div className="flex-1 min-w-0">
        <div className="text-xs font-semibold truncate" style={{color:'var(--text)'}}>{title}</div>
        <div className="text-[11px] mt-0.5" style={{color:'var(--text2)'}}>{sub}</div>
      </div>
  ...
  return (
    <div className="p-4 md:p-6 space-y-5 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold" style={{color:'var(--text)'}}>{greeting}, {user?.first_name}</h1>
          <p className="text-xs mt-0.5" style={{color:'var(--text3)'}}>{org?.name}</p>
        </div>
      </div>

      {/* Trial banner */}
      {isTrial && <TrialBanner daysLeft={trialDaysLeft} onUpgrade={() => navigate('/settings/billing')} />}

      {/* Revenue at risk — hero */}
      <button onClick={() => navigate('/modules/hot-reqs')}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard label="Active billing" value={`$${(activeRev/1000).toFixed(1)}K/mo`}
          color="var(--green)" delta={`${margin?.summary?.activePlacementCount || 0} placements`}
          onClick={() => navigate('/modules/margin')} />
        <StatCard label="Avg margin" value={`${avgMargin.toFixed(1)}%`}
          color={avgMargin < 12 ? 'var(--red)' : avgMargin < 16 ? 'var(--amber)' : 'var(--green)'}
          delta={`${margin?.summary?.belowFloorCount || 0} below floor`}
          onClick={() => navigate('/modules/margin')} />
        <StatCard label="Ending in 30d" value={endingSoon}
          color={endingSoon > 3 ? 'var(--amber)' : 'var(--text)'}
          delta="contracts"
          onClick={() => navigate('/modules/radar')} />
        <StatCard label="Client alerts" value={redClients}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {/* Actions */}
        <div>
          <SectionHead title="⚡ Actions needed today"
            action={() => navigate('/modules/pulse')} actionLabel="Ask Pulse AI" />
          {actions?.length ? (
            ...
            <Empty icon="✅" title="No urgent actions" desc="All requirements and contracts are on track." />
          )}
        </div>

        {/* Critical reqs */}
        <div>
          <SectionHead title="🔥 Critical reqs"
            action={() => navigate('/modules/hot-reqs')} actionLabel="View all" />
          {digest?.criticalReqs?.length ? (
              ...
              {digest.criticalReqs.slice(0,3).map(r => (
                <div key={r.id} className="flex items-center gap-3 p-3 rounded-xl"
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[11px]" style={{color:'var(--text2)'}}>{r.client_name}</div>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <Badge label={`${r.days_open}d`} type={agingBadgeType(r.days_open)} />
                    <div className="text-[11px] font-bold font-mono mt-1"
                      style={{color:'var(--red)'}}>
            ...
            <Empty icon="📋" title="No critical reqs" desc="All open requirements are within acceptable age." />
          )}
        </div>
      </div>

      {/* Contracts ending */}
      {(radar?.byUrgency?.critical?.length > 0 || radar?.byUrgency?.urgent?.length > 0) && (
        <div>
          <SectionHead title="📡 Contracts ending soon"
            action={() => navigate('/modules/radar')} actionLabel="Full radar" />
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            {[...(radar.byUrgency?.critical||[]),...(radar.byUrgency?.urgent||[])].slice(0,3).map(p => (
              <div key={p.id} className="flex items-center gap-3 p-3 rounded-xl"
                style={{background:'var(--elevated)', border:'1px solid var(--border)'}}>
                <span className="text-2xl font-black font-mono flex-shrink-0"
                  style={{color: p.days_to_end <= 7 ? 'var(--red)' : 'var(--amber)'}}>
                  {p.days_to_end}d
                </span>
                <div className="flex-1 min-w-0">
                  <div className="text-xs font-semibold truncate" style={{color:'var(--text)'}}>{p.contractor_name}</div>
                  <div className="text-[11px] truncate" style={{color:'var(--text2)'}}>{p.client_name}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Pulse CTA */}
      <button onClick={() => navigate('/modules/pulse')}
        className="w-full flex items-center gap-4 p-4 rounded-xl text-left transition-all"
        style={{background:'var(--teal-dim)', border:'1px solid rgba(34,211,238,0.2)'}}>
        <PulseDot color="var(--teal)" size={8} />
        <div className="flex-1">
          <div className="text-sm font-bold" style={{color:'var(--teal)'}}>✦ Plaxiq Pulse AI</div>
          <div className="text-xs" style={{color:'var(--text2)'}}>Ask anything about your revenue</div>
        </div>
